...
export class RobinhoodService {
    ...
    return (new Promise((resolve, reject) => {
      this.http.get(this._proxyURL + this._endpoints.positions
        ...
        positions.forEach(position => {
          ...
          instrumentPromises.push(p.catch((error) => {
            return (error)
          }));
        });

        Promise.all(instrumentPromises).then(()=>{
          const quotePromise = this.getQuoteBulk(
            positions.map(pos => pos.instrument.symbol)
          )
          quotePromise.then(quotes =>{
            positions.forEach(position => {
              position.instrument.quote = quotes[position.instrument.symbol];
    ...
    return (new Promise((resolve, reject) => {
      this.http.get(this._proxyURL + this._endpoints.watchlists, {
        ...
        watchList.forEach(watchItem => {
          instrumentPromises.push(this.getInstrument(watchItem).catch((error) => {
            return (error)
          }));
        });
        ...
        Promise.all(instrumentPromises).then(()=>{
          this.getQuoteBulk(
            watchList.map(pos => pos.instrument.symbol)
          ).then(quoteRes => {
            watchList.forEach(watch => {
              watch.instrument.quote = quoteRes[watch.instrument.symbol];
              this.account.recentOrders.forEach(order => {
  ...
  cancelOrder(order: Order) {
    return (new Promise((resolve, reject) => {
      this.http.post(this._proxyURL + order.cancel.replace(RegExp('[^:]*:\/\/[^\/]*\/'), ''), {}, {
        headers: this.setHeaders()
      }).subscribe(res => {
  ...
  /**
   * Get Quotes in bulk with one http call
   * @param {Array<string>} symbols
   * @returns {Promise}
   */
  getQuoteBulk(symbols: Array<string>): Promise<any>{
    // https://api.robinhood.com/quotes/?symbols=MSFT,FB,TSLA
    return(
      new Promise((resolve, reject) => {
        this.http.get(this._proxyURL + this._endpoints.quotes + '?symbols=' + symbols.join(','), {
          headers: this.setHeaders()
        }).subscribe(res => {
          const dict = {};
